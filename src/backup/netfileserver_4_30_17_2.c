...
char *prepMessage(char *messages[], int numMessages) {
  char *messageToSend = NULL;
  int count;
  int reportedMessageLength = 0;
  int totalMessageLength = 0;
  char reportedMessageLengthStr[100];
  for (count = 0; count < numMessages; count++) {
    reportedMessageLength += strlen(messages[count]) + 1; // for a delimiter
  }
  sprintf(reportedMessageLengthStr, "%d", reportedMessageLength);
  totalMessageLength = 2 + strlen(reportedMessageLengthStr) + reportedMessageLength + 1;
...
void *connectionHandler(void *args) {
  ...
  read(fd, restOfContent, len - numBytesReadAfterDelimiter); //how to ensure this happens?
  ...
  switch (type) {
    case OPEN:
      ...
      if (newFd == -1) { // error opening file
        ...
        char *messageComponents[] = {networkFdString};
